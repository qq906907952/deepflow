...
use tcp_reassemble::payload::{Payload, PayloadReader};
...
impl FlowLog {
    ...
    fn l7_parse_log(
        ...
        let mut logs = None;
        while let Some((payload, can_reassemble)) = payload_reader.get() {
            parse_param.set_payload_can_reassemble(can_reassemble);
            match parser.parse_payload(payload, parse_param) {
                Ok(res) => {
                    if logs.is_none() {
    ...
    fn l7_check(
        ...
        */
        while let Some((payload, can_reassemble)) = payload_reader.get() {
            param.set_payload_can_reassemble(can_reassemble);
            'check: for protocol in checker.possible_protocols(
                ...
                match parser.check_payload(payload, &param) {
                    ...
                    CheckResult::NeedMoreData => {
                        let mut idx = 1;
                        while let Some((p, can_reassemble)) = payload_reader.peek(idx) {
                            param.set_payload_can_reassemble(can_reassemble);
    ...
    fn get_payload_reader<'a>(
        ...
        let direction = meta_packet
            .lookup_key
            .get_reassemble_direction(ip_port_0.0, ip_port_0.1);
        if tcp_reassemble.buf_is_empty(direction) {
            ...
            return Some(PayloadReader::new(Payload::MetaPacket(
                ...
                (cut_payload, cut_payload.len() != log_size),
            )));
        } else {
            ...
            match tcp_reassemble.reassemble_non_serial(tcp_data.seq, cut_payload, direction)
            {
                Ok(_) => {
                    if tcp_reassemble
                ...
                Err(e) => match e{
                    tcp_reassemble::tcp_reassemble::TcpReassembleError::FrameBeforeBase => {
                        param.set_payload_in_buffer(true);
                        Some(PayloadReader::new(Payload::MetaPacket((cut_payload,false))))
                    },
    ...
    pub fn new(
        ...
        Some(Self {
            ...
            ntp_diff,
            ip_port_0,
            tcp_reassemble: if l4_proto == L4Protocol::Tcp && max_tcp_frame > 1 {
        ...
        if l7_performance_enabled || l7_log_parse_enabled {
            ...
            let res = if let Some(tcp_reassemble) = self.tcp_reassemble.clone() {
                ...
                let Some(payload_reader) =
                    Self::get_payload_reader(&mut t, packet, log_size, param, self.ip_port_0)
                else {
                    return Err(Error::L7ProtocolParseLimit);
                };
                ...
                if drain_frame != 0 {
                    t.drain_frames(
                        lookup_key.get_reassemble_direction(self.ip_port_0.0, self.ip_port_0.1),
                        drain_frame,
                    );
                }
                ...
                if let Err(Error::NeedMoreData) = res {
                    ...
                    */
                    if !param.payload_in_buffer {
                        // 如果 payload_in_buffer 目前的逻辑重组 buffer 必定是空, drop_frame 必定是0
                        match t.reassemble_non_serial(param.tcp_seq, packet.get_l4_payload().unwrap(), lookup_key.get_reassemble_direction(self.ip_port_0.0, self.ip_port_0.1)){
                        Ok(_) => {},
                        Err(e) => match e{
                ...
                let (res, _) = self.l7_parse(
                    PayloadReader::new(Payload::MetaPacket((
                        packet.get_l4_payload_cut(log_size).unwrap(),
                        false,
                    ))),
                    param,
                    &mut lookup_key,
