...
pub enum Payload<'a> {
    ...
    // (payload, can reassemble?)
    MetaPacket((&'a [u8], bool)),
...
impl<'a> PayloadReader<'a> {
    ...
    */
    ...
    // return payload, whether can reassemble.
    pub fn peek(&self, n: usize) -> Option<(&[u8], bool)> {
        match &self.payload {
            Payload::MetaPacket((payload, can_reassemble)) => {
                if self.buffer_idx + n > 0 {
                    None
                } else {
                    Some((payload, *can_reassemble))
                }
            }
            Payload::InFlightBuffer(buffer, tcp_frames) => {
                ...
                Some((
                    &(buffer.to_slice()[self.payload_start..payload_off]),
                    // inflight data can reassemble
                    true,
                ))
            }
            ...
            Payload::FlushedBuffer(buffer) => {
                ...
                Some((
                    &buf.as_slice()[self.payload_start..payload_off],
                    self.tcp_frame_off + n < tcp_frames.len(),
                ))
            }
        }
    }
