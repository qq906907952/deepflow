...
fn test_payload_exceed() {
    ...
    let mut buf = BufferData {
        ...
        direction: 0,
        buf: RingBuf::new(16),
        tcp_meta: Vec::with_capacity(8),
        base_seq: None,
        max_frame: 8,
    };

    let _ = buf.reassemble(f0.0.seq, f0.1.as_slice());
    let _ = buf.reassemble(f1.0.seq, f1.1.as_slice());
    ...
    match buf.reassemble(exceed_frame.0.seq, exceed_frame.1.as_slice()) {
        Ok(_) => unreachable!(),
        Err(e) => match e {
            TcpReassembleError::BufferFlush(mut f_buf) => {
                // the test case againest case 3.1.1.2
                assert_eq!(f_buf.len(), 3);
                let (payload, mut frames) = f_buf.remove(0);
                assert_eq!(frames.remove(0), f0.0);
                assert_eq!(payload, f0.1);

                let (payload, mut frames) = f_buf.remove(0);
                assert_eq!(frames.remove(0), f1.0);
                assert_eq!(payload, f1.1);

                let (payload, mut frames) = f_buf.remove(0);
                assert_eq!(frames.remove(0), exceed_frame.0);
                assert_eq!(payload, exceed_frame.1);
            }
            _ => unreachable!(),
        },
    }
